...
/// Enables a [Source](rodio::Source) to be recovered after playing it through a
/// [Sink].
///
/// Normally, a [Source] cannot be accessed again after supplying it to a
/// [Sink], which can be annoying when you want to continue a partially played
/// sound from another [Sink]. Wrapping it in this type lets you work around
/// that.
///
/// When this type is dropped, for example by calling [stop](rodio::Sink::stop)
/// on its [Sink] or letting its [Sink] go out of scope, it is sent to its
/// corresponding [Retriever](Retriever) instance and can be retrieved from
/// there.
///
/// [Source]: rodio::Source
/// [Sink]: rodio::Sink
...
impl<S> Drop for Recoverable<S> {
    fn drop(&mut self) {
        if let Some(src) = self.source.take() {
            // ignore any errors, because if the receiver was dropped, they probably
            // didn't want the source back anyway
            self.sender.send(src).ok();
        }
    }
}
    ...
    S::Item: Sample,
{
    fn current_frame_len(&self) -> Option<usize> {
        self.source.as_ref().and_then(<_>::current_frame_len)
    }

    fn channels(&self) -> u16 {
        self.source.as_ref().map(<_>::channels).unwrap_or_default()
    }
    ...
    fn total_duration(&self) -> Option<std::time::Duration> {
        self.source.as_ref().and_then(<_>::total_duration)
    }
}
...
impl<S: Iterator> Iterator for Recoverable<S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.source.as_mut().and_then(<_>::next)
    }
}
